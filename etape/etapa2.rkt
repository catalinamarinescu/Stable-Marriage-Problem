...
(define (get-women wpref)
  (foldr (lambda (minList result) (cons (car minList) result)) null wpref))
...
(define (get-pref-list pref person)
  (cdr (car (filter (lambda (minList) (equal? person (car minList))) pref))))
...
(define (get-partner engagements person)
  (define (f L)
    (if (equal? person (car L))
        #t
        #f))
  (define x (find-first f engagements))
  (if (equal? x #f)
      #f
      (cdr x)))
...
(define (update-engagements engagements p1 p2)
  (define (f L)
    (if (equal? p1 (car L))
        #t
        #f))
  (change-first f engagements (cons p1 p2)))
...
(define (stable-match? engagements mpref wpref)
 (define save engagements)
 (define (f L)
   (if (equal? (or (better-match-exists? (cdr L) (car L) (get-pref-list mpref (cdr L)) wpref engagements)
                    (better-match-exists? (car L) (cdr L) (get-pref-list wpref (car L)) mpref engagements)) #f)
       #t
       #f))
  (if (equal? (filter f engagements) save)
      #t
      #f))